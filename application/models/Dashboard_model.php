...
class Dashboard_model extends CI_Model
{
    ...
    public function getJumlahIesp()
    {
        $this->db->select('COUNT(*) AS total_iesp');
        $this->db->from('user');
        $this->db->where('user.prodi', 'iesp');
        $this->db->where('user.status', 2);
        $sum = $this->db->get()->row_array();
        return $sum;
    }

    public function getJumlahMana()
    {
        $this->db->select('COUNT(*) AS total_mana');
        $this->db->from('user');
        $this->db->where('user.prodi', 'Manajemen');
        $this->db->where('user.status', 2);
        $sum = $this->db->get()->row_array();
        return $sum;
    }
    ...
    {
        $this->db->select('COUNT(*) AS total_akun');
        $this->db->from('user');
        $this->db->where('user.prodi', 'Akuntansi');
        $this->db->where('user.status', 2);
        $sum = $this->db->get()->row_array();
        return $sum;
    }

    public function getAntriIesp()
    {
        $query = "SELECT COUNT(*) as antri_iesp FROM antri WHERE  aprodi='iesp' AND status=0";
        return $this->db->query($query)->row_array();
    }

    public function getAntriMana()
    {
        $query = "SELECT COUNT(*) as antri_mana FROM antri WHERE  aprodi='manajemen' AND status=0";
        return $this->db->query($query)->row_array();
    }

    public function getAntriAkun()
    {
        $query = "SELECT COUNT(*) as antri_akun FROM antri WHERE  aprodi='akuntansi' AND status=0";
        return $this->db->query($query)->row_array();
    }
    ...
    {
        $query = "SELECT COUNT(*) as totalkh FROM tb_periksa WHERE tanggal=CURRENT_DATE()";
        return $this->db->query($query)->row_array();
    }
